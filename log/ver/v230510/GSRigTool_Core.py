...
def AdvancedSekelton5():
    avs_path = 'source "/gstepasset/WorkLibrary/1.Animation_team/Script/_forRigger/GSRigTool/util/Tools/AdvancedSekeltonFix_9/AdvancedSkeleton5.mel"'
    mel.eval(avs_path)
...
def fitBuild():
    ...
    # nurbs = pm.listConnections('FitSkeleton.visNurbs')
    # nurbs_grp = pm.group(nurbs, n='guide_nurbs')
    # pm.parent(nurbs_grp, w=1)
...
def autoOrientSetup():
    ...
    with pm.UndoChunk():
        ...
        for finger in pm.ls('Wrist*', dag=True):
            if pm.nodeType(finger) == 'joint':
                finger = pm.PyNode(finger)
        ...
        for scapula in pm.ls("Scapula*"):
            if pm.nodeType(scapula)=='joint':
...
def autoOrientFingerSetup(jntL):
    ...
    for finger in pm.ls('Wrist*', dag=True):
        if pm.nodeType(finger) == 'joint':
            finger = pm.PyNode(finger)
            pm.setAttr(finger.freeOrient, 0)
...
def rightAfterBuild():
    ...
    pm.move(swinger, -0.15,-1.25,0, os=True, wd=True)
...
def RunFolSubRigTool():
    ...
    from util.Tools import folSubRigTool
...
def RunBinSimpleRigTool():
    ...
    from util.Tools.subRigTool import simpleRigTool
...
def RunPropRigTool():
    ...
    from util.Tools.subRigTool import propRigTool_Run
