...
class SkinWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # main_layout.addLayout(self.popupItem_TEST())
    ...
    def SkinUI(self):
        ...
        self.avgSmooth_plg_btn = QtWidgets.QPushButton("AvgSkin", self, fixedHeight=45, font=nfont)
        self.smoothSkin_plg_btn = QtWidgets.QPushButton(self, icon=QtGui.QIcon(icon_path+"/maya_icons/brSmoothWeights.svg"), iconSize=QtCore.QSize(31,35))
        self.ngSkin_plg_btn = QtWidgets.QPushButton(self, icon=QtGui.QIcon(icon_path+"/maya_icons/ngSkinTool.png"), iconSize=QtCore.QSize(31,35))
        ...
        # CONNECT
        self.driver_replace_btn.clicked.connect(self.driverReplaceItemList)
        self.driver_remove_btn.clicked.connect(self.driverRemoveItemList)
        self.driven_replace_btn.clicked.connect(self.drivenReplaceItemList)
        self.driven_remove_btn.clicked.connect(self.drivenRemoveItemList)
        self.copyToOne_btn.clicked.connect(self.CopySkinOneToOne)
        self.copyToAll_btn.clicked.connect(self.CopySkinOneToAll)
    ...
    def driverReplaceItemList(self):
        scroll = self.driver_scroll
        scroll.clear()
    ...
    def driverRemoveItemList(self):
        scroll = self.driver_scroll
        scroll.clear()


    def drivenReplaceItemList(self):
        scroll = self.driven_scroll
        scroll.clear()
    ...
    def drivenRemoveItemList(self):
        scroll = self.driven_scroll
        scroll.clear()

    
    def CopySkinOneToOne(self):
        
        driver_scroll = self.driver_scroll
        driver = [driver_scroll.item(x).text() for x in range(driver_scroll.count())]
        
        driven_scroll = self.driven_scroll
        driven = [driven_scroll.item(x).text() for x in range(driven_scroll.count())]
        
        core.CopySkinOneToOne(driver, driven)
        
    
    def CopySkinOneToAll(self):
        ...
        core.CopySkinOneToAll(driver, driven)
